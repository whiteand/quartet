...
const variantCheck = (variantsConfigs, registered) => (obj, ...parents) => {
  return variantsConfigs.some(config => {
    ...
    if (Array.isArray(config)) {
      return config.every(andVariant => where(andVariant, registered)(obj, ...parents))
    }
    return where(config, registered)(obj, ...parents)
  })
}
function validateConfig(config) {
  if (!['string', 'function', 'object'].includes(typeof config) || config === null) {
    throw new TypeError('config must be either name of registered config, isValid function or object config')
  }
...
function validateRegistered(registered) {
  if (typeof registered !== 'object' || registered === null) {
    throw new TypeError('registered must be an object { key1: config1, key2: config2, ...}')
  }
}
const getType = x => {
...
function where (config, registered = {}) {
  ...
  switch (getType(config)) {
    case 'string': return stringCheck(config, registered)
    case 'function': return functionCheck(config, registered)
    case 'object': return objectCheck(config, registered)
    case 'array': return variantCheck(config, registered)
  }
}
